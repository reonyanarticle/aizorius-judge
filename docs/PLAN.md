...
| Phase | 内容 | 完了条件 |
|---|---|---|
| **0. データ基盤＋ゴールデンデータセット** | CR原文の入手・**利用条件の一次確認**・版固定（発効日＋ハッシュ）、`parse_rules.py`（CR→JSON）、評価データセット15問の作成・**出典検証・人間承認**、Embeddingモデルのスパイク検証（bake-off） | dataset 15問が出典検証済み＋人間承認済み／CR JSONが生成できる／Embedding方針が確定 |
...
| **3. 統合評価＋拡充** | dataset 15→52問、`test_runner.md`、eval-runner による採点・回帰検知の運用開始、外部評価（任意） | 各問スコア≥7/10・全体精度80%以上 |
...
4. **ゴールデンデータセット15問**（`evaluation/dataset.json`）：スキーマは [EVALUATION.md](EVALUATION.md) §3。dataset-curator サブエージェントが**CR原文と照合して出典検証**し、**正解としての最終承認は人間**が行う（LLM生成の正解をLLMで採点する自己参照を避ける）。カテゴリは basic_rules / stack_priority / commander から開始。
5. **Embeddingスパイク（bake-off）**：`paraphrase-multilingual-MiniLM-L12-v2` と対抗候補（multilingual-e5 等）1つ以上で CR の一部（100〜500ルール）をインデックスし、dataset の日英クエリで recall を粗く測って**モデルとインデックスの言語戦略（日英併記/別建て）を決定**する（コア実装に入る前に方式リスクを潰す）。
