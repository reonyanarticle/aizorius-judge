...
| **1.8. データオーギュメンテーション（外部Q&Aの取り込み）** | 外部のMTGルールQ&Aを**ライセンス互換の一次確認のうえ**取り込み、評価資産を拡充する（プロジェクトはOSSとして公開する方針＝ユーザー決定 2026-07-04）。①~~プロジェクト自体のライセンス確定~~ **済**（2026-07-04 ユーザー決定：リポジトリ全体 MIT・LICENSE 設置・README に Fan Content 文言・外部データは CC0/CC BY 優先。基準の正本は [ARCHITECTURE.md](ARCHITECTURE.md) §5）。CR由来物のコミット不可は従来どおり維持 ②~~ソースごとの利用条件の一次確認~~ **済**（2026-07-04。採用したのは公式リリースノートFAQと Stack Exchange の2ルート。基準の正本は [ARCHITECTURE.md](ARCHITECTURE.md) §5、調査経緯は PR/コミット履歴） ③**取り込み設計**：Stack Exchange は対象タグの個別取得で **hold-out 口語クエリ集**（ローカル限定＋帰属保持）として使う。dataset 本体へ入れる場合は言い換え＋CR照合＋**人間承認**の既存運用（[EVALUATION.md](EVALUATION.md) §3） ④**用途**：手薄カテゴリの評価問拡充（**FAQ候補15問起案済み・人間承認待ち** → `evaluation/reviews/REVIEW-faq.md`）／**hold-out 口語クエリ集**＝~~運用開始~~ **済**（2026-07-04：SE 350問取得・弱ラベル被覆の初回測定と実例検分まで完了。レポート: `evaluation/reports/holdout-eval.md`。スクリプト: `fetch_holdout_queries.py` / `eval_holdout.py`）／LLM-judge較正の追加サンプル | LICENSE 設置済み・全ソースの利用条件が一次情報で確認/記録済み（達成 2026-07-04）／hold-out 運用開始済み（達成 2026-07-04）／**残**: FAQ候補15問の人間承認（`evaluation/reviews/REVIEW-faq.md`） |
