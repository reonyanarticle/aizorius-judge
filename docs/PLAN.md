...
| **1.8. データオーギュメンテーション（外部Q&Aの取り込み）** | 外部のMTGルールQ&Aを**ライセンス互換の一次確認のうえ**取り込み、評価資産を拡充する（プロジェクトはOSSとして公開する方針＝ユーザー決定 2026-07-04）。①**プロジェクト自体のライセンス確定が前提**：現状 LICENSE 未設置。OSSライセンスを決めて設置し、コードとデータ（evaluation/dataset.jsonl）を同一ライセンスにするか別にするか（例: コード=MIT・データ=CC BY-SA）も決める。CR由来物のコミット不可（Fan Content Policy → [ARCHITECTURE.md](ARCHITECTURE.md) §5）は従来どおり維持 ②**ソースごとの利用条件の一次確認**（推測で進めない）：RulesGuru（約1,500問・ジャッジ品質）は利用規約で**ML利用に許諾が必要**＝ライセンス以前に許諾取得の問い合わせが要る／公開データセット（MTG-Eval 等）は配布ライセンスを原文で確認。判定基準: CC BY・CC BY-SA・GPL系＝帰属・継承の条件を満たして取り込み可、CC BY-NC 等の非商用限定＝OSS配布と衝突するため不可。ソースごとの判定結果と根拠URLを記録 ③**取り込み設計**：外部Q&A→dataset スキーマ（[EVALUATION.md](EVALUATION.md) §3）への変換。`source` にライセンス・出典を明記し、逐語コピーでなく言い換え＋CR照合。dataset-curator の出典検証＋**人間承認**の既存運用を変えない ④**用途**：手薄カテゴリの評価問拡充／**hold-out 口語クエリ集**（検索改善の汎化チェック用。課題一覧の「口語同義語は hold-out 検証が条件」の前提を満たす）／LLM-judge較正の追加サンプル | 取り込む全ソースのライセンス・利用許諾が一次情報で確認・記録済み／LICENSE 設置済み／拡充分も出典検証＋人間承認済み／hold-out 口語クエリでの検索汎化測定が運用に入る |
