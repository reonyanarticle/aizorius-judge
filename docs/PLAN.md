...
| **1.6. 評価フィードバックの反映（検索改善・バグ修正・dataset拡充）** | 生成層評価・LLM-judge較正・コード/設計レビュー（2026-07-04）で見つかった課題の解消。①**重大バグ2件の修正＋単体テスト補強**：ルール番号直接引きの前方一致境界（"702.9"に702.90〜99が混入）／用語集の複数形・範囲参照（"rules 509.1b–c"）の抽出漏れ。テスト0件だった `group_by_parent`・直接引き・用語集パーサに専用テストを追加 ②**検索改善**（優先順）：セクション内 timing/定義ルール（704.3/903.3等）の決定論的同伴 → 用語集明示ヒットの最終グループ枠保証。各施策は recall/MRR の劣化を同時計測して採否 ③**評価プロトコル改善**：言い換え反復は実LLMに元質問だけ渡して生成させる（手書き固定クエリは評価汚染のため不可） ④**card_interactions カテゴリ15問**（`evaluation/dataset-candidates-cards.jsonl`）の人間承認と dataset 統合（検索単体評価の母集団からは除外） ⑤**LLM-judgeルーブリックの調整判断**（引用重視を維持 or 実質正解時の引用減点を緩和→再採点。ユーザー判断） | 重大バグ2件が修正されテストで再発防止／検索改善後の must_cite recall@5 が 0.850 から劣化せず、検索起因の不合格4問のうち2問以上を回収／card_interactions が人間承認済みで統合 |
