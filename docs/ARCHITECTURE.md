...
sequenceDiagram
    ...
    C->>M: search_rules("到達")　※情報不足なら追加検索（十分性基準・回数上限なし → §6）
...
- **BM25 は日英併記テキスト**に対して、形態素解析器なしのトークナイザ（英数字は単語・ルール番号は1トークン・日本語は文字バイグラム・英語は軽量ステミング＝triggers/triggered を同一トークン化）で構築。
...
- **Reranker：`BAAI/bge-reranker-v2-m3`（多言語・決定）**。設計原案候補の `ms-marco-MiniLM-L-6-v2`（英語学習）は多言語要件に合わず不採用。`RERANKER_MODEL` を空にすると rerank なしの高速構成になる。入力は max_length=1024 / batch=8（512だと長文ルールの日本語側が切られたまま採点される。1024で must_cite@7 +0.004・MRR +0.007・OOMなし・レイテンシ悪化なしを実測して採用）。
- **rerank まわりの棄却実験（実測記録・2026-07-05〜06）**: miss の機構別棚卸しで「候補プールに入るが rerank が落とす」型が golden misses の64%を占めたため、①融合順位とのブレンド ②passage の日本語優先 ③グループ単位リランク を順に試したが、**3案とも golden の must_cite が劣化して棄却**（①0.859→0.845 ②0.905→0.886 ③0.859→0.705）。単一クエリの検索はローカルモデル構成での局所最適に近く、深い取り逃しの回収はクライアントの反復検索（言い換え込みで must_cite 0.995）が担う設計を維持する。
...
- **実測性能と構成の使い分け**（M4/MPS・110問・[../evaluation/reports/retrieval-tuning.md](../evaluation/reports/retrieval-tuning.md)）：
  ...
  |---|---|---|---|
  | 既定（rerank あり）・単一クエリ・k=5 | 0.692 | 0.859 | 0.792 |
  | 既定・単一クエリ・k=7 | 0.755 | 0.909 | — |
...
- **返却グループ数の既定は7**：グループ数感度の実測は k=5: 0.692/0.859 → k=7: **0.755/0.909** → k=10: 0.778/0.914（単一クエリ・recall/must_cite）。k=10 の上積み +0.005 はコンテキスト +43% に見合わず、既定は k=7 を維持（必要ならクライアントが max_results で広げる）。
...
- 外部Q&Aデータの取り込みは**利用条件を一次情報で確認してから**（推測で進めない）。判定基準:
  ...
  - ライセンスとは別に**利用規約の制約**（ML利用に個別許諾を求めるサイト等）はライセンス互換でも許諾取得が必要。
- 取り込み時の品質運用は評価データセットの既存規約のまま（逐語コピーなし・言い換え＋CR照合・人間承認 → [EVALUATION.md](EVALUATION.md) §3）。
- 現在使っている外部ソースは2つ: **公式リリースノートFAQ**（Fan Content Policy の範囲で言い換え）と **boardgames.stackexchange.com**（CC BY-SA 4.0・帰属つき・ローカル限定の hold-out クエリ用途。公式ダンプでなく対象タグの個別取得）。個別ソースの調査経緯は PR/コミット履歴に記録。
