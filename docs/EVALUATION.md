...
| 層 | 基準 |
|---|---|
| 検索単体（主指標） | **must_cite recall@5 ≥ 0.8**（裁定の核となるルールが top5 グループに入る割合。実測 0.859 単一クエリ / 実運用相当（実LLM言い換え込み反復・k=7）**0.995**） |
| 検索単体（追跡指標） | recall@5（完全集合ベース・実測 0.692、目標 0.6 達成）と MRR（実測 0.785）を継続計測 |
| 統合評価 | 各問スコア ≥7/10 で合格、全体精度 80% 以上を目標（生成層評価＝ライブラリ直呼びの実測 **100%**・平均9.54。初回96.4%→検索改善＋評価プロトコル改善後の再評価で検索起因4問を回収。サマリは `evaluation/reports/generation-eval.md`） |

- 検索単体の測定は**単一クエリ（下限）と反復検索模擬（実運用相当）の両方**で行う（`scripts/eval_retrieval.py --multi`）。実運用ではクライアントLLMが角度を変えたクエリを反復発行するため、単一クエリの数値は保守的な下限になる。反復検索模擬には決定論的キーワードに加えて**実LLM生成の言い換えクエリ**（`--rephrasings`。質問文のみを入力に生成・golden 遮断・ルール番号禁止）を用いる——手書きの言い換えは答えの先読みで評価を汚染するため使わない（生成規約は `evaluation/test_runner.md`）。

- **層間ゲート**：検索単体（第1層）が基準未達のうちは、統合評価（第3層）のスコア改善に投資しない（検索が正解を返せない状態で推論側を調整しても徒労になる）。
- **LLM-judge の較正**：統合評価（第3層）の初回実行時に15〜20問を人間も採点し、eval-runner の採点との一致率（±1点以内の割合）を確認して採点プロンプトを較正する。LLM採点を人間採点で統計的に裏付ける構成は RAG 評価の先行研究（ARES の PPI 等）に倣う。生成層評価での実測（16問・2026-07-04）：**合否一致 87.5%・±1点一致 56.3%、LLM採点は人間より系統的に辛い**（must_cite 欠落の減点が人間評価より重い）。分析は `evaluation/reports/generation-eval.md`。較正を受けたルーブリック判断（2026-07-04 ユーザー決定）：**引用重視の採点基準を維持する**（ハルシネーション対策を売りとする以上、実質正解でも根拠引用の欠落は減点のまま。人間採点との乖離は「LLM採点は保守的な下限」として解釈する）。
...
|---|---|
| `question` | ルール質問（日本語） |
| `cards` | 設問が参照するカード名の日英ペア（任意。`[{"ja": "血染めの月", "en": "Blood Moon"}]`。`lookup_card` 入力を機械可読にし、Scryfall再検証を容易にする。card_interactions で使用） |
...
| `card_interactions` | 15 | 個別カードとルールの相互作用（血染めの月・ドラニスの判事・倍増の季節等。`lookup_card`/`get_card_rulings` が主役）。**検索単体評価（第1層）の母集団からは除外**（`eval_retrieval.py` でフィルタ。lookup_card 主体の問を混ぜると recall 指標が歪むため）。統合評価（第3層）には含める |
