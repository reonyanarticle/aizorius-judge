...
| 層 | 基準 |
|---|---|
| 検索単体（主指標） | **must_cite recall@5 ≥ 0.8**（裁定の核となるルールが top5 に入る割合。Phase 1 実測 0.805 で達成） |
| 検索単体（追跡指標） | recall@5（完全集合ベース）と MRR を継続計測（Phase 1 実測 0.519 / 0.753）。完全集合 recall@5 は平均3.1件の正解を全て top5 に入れる厳しい定義のため、目標 0.6 を次の改善目標とする |
